...
| Lock, unlock, and key lifecycle | Match GALVANIZE's locked-start behavior: report health while locked, keep query/replication listeners and workers unavailable until unlock, accept secrets through a protected API or callback, keep key material only in zeroized memory, and reject bad keys without crashing. Specify offline MariaDB rekey and recovery procedures; the FIFO key handoff alone does not cover these workflows. |
...
| Operator CLI and node lifecycle | Inventory and map `agent`, `cluster`, `sync`, `reload`, `backup`, `restore`, `rekey`, `tls`, `query`, `subs`, and Consul commands to package APIs or a companion Go CLI. Include bootstrap/join, drain/retire, status, and recovery workflows with documented authorization. |
...
- **Current implementation versus target:** The product target is a Go package that controls MariaDB startup/configuration and replication. The current package instead takes an existing `*sql.DB`, injects a generic logger, and exposes migration SQL for the host to install; it does not implement the planned process manager, config/key lifecycle, package-owned DDL, Zap/timberlog logger, or GALVANIZE-facing services. Keep these as explicit roadmap gaps. In managed mode the API must define how MARIAMESH obtains its SQL connection after launch (socket/TCP address, database/user credentials or a secret-provider callback); a required pre-opened `*sql.DB` cannot be the only input if the package owns server startup. Define whether the process is merely started or actively supervised, what happens on `Close`, crash, and failed startup, and how datadir ownership and MariaDB shutdown are handled. An attach mode may remain as a clearly separate option for an externally managed daemon. Make config rewriting opt-in and auditable, or fail with a precise remediation report; never silently rewrite a production MariaDB config. Align README examples and API with the selected modes, and label target architecture separately from shipped behavior.
- **Parity source and scope:** This review used the restored sibling `../GALVANIZE` checkout and the active Go client at `/home/marc/OVERWATCH v2.2/backend/pkgs/galvanize`. Recheck against deployed configurations before cutover. Prioritize capabilities actually used by deployed Go applications and operators; do not imply that matching the replication protocol alone replaces GALVANIZE's service, CLI, and operations surfaces.
...
3. Run equivalent live scenarios on MariaDB: partitions and heal, concurrent updates/deletes, crash recovery, large blobs, peer allow-list, locked startup/unlock/rekey, schema drift and reload (including views), High/Low corruption/replay/gaps and role behavior, file peer fetch and air-gap delivery, backup/restore/reseed, and Go client/operator workflows. Compare logical row and file contents across nodes; byte-identical database files are not a meaningful MariaDB criterion.
